"""
MHT Agentic - Demo Mode

Starts monitoring in demo mode with minimal overlays:
- Bottom-center status bar (inbound/outbound status)
- Top-left extracted data panel (no pharmacy field)

All other overlays (patient log, roomed, discharged, WR border,
row borders, control panel, analytics) are hidden.
...
print('Starting DEMO monitoring (inbound + outbound)...')
print('Minimal overlays: status bar + extracted data panel')
print()

# Run the full monitoring function in DEMO MODE
launcher._start_monitoring(control, outbound_worker=outbound, demo_mode=True)
